...
// Note, this is a small sized FFT, so this setup object can be easily used for other small ffts.
// But for large FFTS, the setup object may cause bad performance when applied to smaller problems
// so it is important to watch for that.
...
// below is some really cool Swift magic. It creates arrays of size halfN of all zeros.
...
// (Salman)- someone explain this to me please!
// "Because each complex value stores two real values, the length of teach array is half that of signal."
// Maybe has something to do with: "The conversion stores the even values in signal as the real components 
// in forwardInput, and the odd values in signal as the imaginary components in forwardInput."
...
2.Use convert(interleavedComplexVector:toSplitComplexVector:) to convert the real values in signal to complex 
numbers. The conversion stores the even values in signal as the real components in forwardInput, and the odd 
values in signal as the imaginary components in forwardInput.

3.Create a DSPSplitComplex structure with pointers to forwardOutputReal and forwardOutputImag to receive 
the FFT result.

4.Perform the forward FFT.
*/

// (Salman) - It seems that we use withUnsafeMutableBufferPointer and array.baseAddress to make this super
// robust, but tbh I don't fully understand when and why we would use it otherwise.
...
// The forwardOutputImag (or the Nyquist components of the foward FFT) contain the frequencies of the input!
...
// We could also recreate the signal - check the guide if that is something you want to do (probably don't for this project)